...
const Manager = () => {
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      const url = form.id 
        ? `${API_BASE_URL}/passwords/${form.id}`
        : `${API_BASE_URL}/passwords`

      const response = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, id: form.id || uuidv4() })
      })

      if (!response.ok) throw new Error('Failed to save')

      toast.success(`Password ${form.id ? 'updated' : 'saved'} successfully`)
                    ...
                    <div className='flex flex-col gap-4'>
                        ...
                        <div className='relative'>
                            ...
                            <input 
                                ...
                                id="password" 
                            />
                            <span onClick={showPassword} className='absolute right-3 bottom-2 cursor-pointer'>
                                <img ref={ref} width={20} src="icons/eye.png" alt="toggle visibility" />
                            </span>
                        </div>
                    </div>
